  ...
  <table class="table table-compact mt-2 w-full">
    ...
    <tbody>
      <tr v-for="(pid, index) in homework.problemIds">
        <td>{{ index + 1 }}</td>
        <td>
          <router-link class="link" :to="`/courses/${$route.params.name}/problem/${pid}`">
            {{ pid }}
          </router-link>
        </td>
        <td>
        ...
        <td>
          <div class="tooltip" data-tip="Stats">
            <router-link
              ...
              :to="`/courses/${$route.params.name}/problem/${pid}/stats`"
            >
        ...
        <td v-if="session.isAdmin">
          <div class="tooltip" data-tip="Copycat">
            <router-link
              class="btn btn-ghost btn-xs"
              :to="`/courses/${$route.params.name}/problem/${pid}/copycat`"
            >
